...
(function definer(definition) { 
  ...
  // binary search in |array| with API this.visitAny(entry)
    ...
    //    |<------[xxxxxxxx]-------->|
    //      behind     0      ahead
    UglyTreeLeafFinder.getDistanceToMark = function(uglyNodeWithToken) {
      var distanceBehind = uglyNodeWithToken.start.pos - this.mark; // start is inclusive
      if (distanceBehind > 0) {
        return -distanceBehind;
      }
      var distanceAhead = this.mark - (uglyNodeWithToken.end.pos - 1);  // end is exclusive
      if (distanceAhead > 0) {
  ...
  UglyTreeLeafFinder.visitPathToIndex = function(zeroBasedCharacterIndex, uglifyAstArray, fncOfArrUglifyArray) {
    ...
    walker.mark = zeroBasedCharacterIndex;
