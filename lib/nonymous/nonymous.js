...
// Root to leaf pass, from statements to branches looking for functions
  ...
  uglyWalker.pushParent = function(uglyArray) {
    ...
    if (false && Nonymous.debug) {
      console.log("namingStack("+this.namingStack.length+") pushed "+this.getType(uglyArray));
    }
  }

  uglyWalker.popParent = function() {
    var popped = this.namingStack.pop();
    if (false && Nonymous.debug) console.log("namingStack("+(this.namingStack.length)+") popped "+this.getType(popped));
  }
  ...
  uglyWalker.seekInStatement = function(statement) {
    ...
    if (type === 'function' || type === 'defun') {
      setFunctionName(statement, this.namingStack);
    }
