...
// Root to leaf pass, from statements to branches looking for functions
  ...
  function pushParent(uglyArray) {
    ...
    if (false && Nonymous.debug) {
      console.log("namingStack("+namingStack.length+") pushed "+uglyWalker.getType(uglyArray));
    }
  }

  function popParent() {
    var popped = namingStack.pop();
    if (false && Nonymous.debug) console.log("namingStack("+(namingStack.length)+") popped "+uglyWalker.getType(popped));
  }
  ...
  var CATCH = function(val) {
    ...
    pushParent(uglyWalker.getType(val));
  ...
  function getBranches(statement) {
    var statementType = uglyWalker.getType(statement);
  ...
  function seekFunctionsInStatement(statement) {
    var type = uglyWalker.getType(statement);
  ...
  // so we add a case
  function getNameExpression(declFunctionCallOrAssignment, parents) {
    var name = "";
    var type = uglyWalker.getType(declFunctionCallOrAssignment);
    if (type === 'decl') { // 'decl': ['left', LITERAL, 'right', NODE],
      ...
      var name = declFunctionCallOrAssignment[1];  // prop name
      var parentIndex = parents.indexOf(declFunctionCallOrAssignment);
