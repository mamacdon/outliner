...
(function definer(definition) { 
    ...
    if (typeof define === "function") {
        define(['lib/nonymous/UglyWalker.js'], function (UglyWalker) {
            return definition(UglyWalker);
        });
    // CommonJS
    ...
    // <script>
        ...
        Nonymous = definition(UglyWalker);
    }

}(function definition() {

if (arguments.length === 3) {  // untested, I'm just guessing
  var uglyWalker = arguments[0].call('UglyWalker');
  var exports = arguments[1];
  var module = arguments[2];
} else {
  var uglyWalker = arguments[0];
}
...
var Nonymous = {
  ...
  function  getHierarchyName(namingStack) {
    ...
    namingStack.forEach(function findScope(uglyArray){
        ...
        if (type === 'function' || type === 'defun') {
          ...
          if (type === 'pair') {
            scopeName = uglyArray[1]+ Nonymous.isPropertySep;  
          } else if (type === 'decl') {
            scopeName = uglyArray[1]+ Nonymous.isPropertySep;  
          } else if (type === 'assign') {
            scopeName = generateName(uglyArray[2]) + Nonymous.isPropertySep;
          }
        } 
        if (scopeName) {
          if (Nonymous.debug === 'trace') {
  ...
  function setFunctionName(statement, namingStack) {
    ...
    if (!name) { // anonymous function
      ...
      var parent = nameObj.parent;
      namingStack = nameObj.namingStack;
      hierarchy = getFullHierarchy(name, namingStack, parent);
    } else {
      hierarchy = getFullHierarchy(name, namingStack, namingStack[0]);
    }
    ...
    var fullName = hierarchy.join('');
    Nonymous.names.push({name:fullName, hierarchy: hierarchy, localName: name, line: loc.line, col: loc.col, pos: loc.pos});
  ...
  var getObjectSummary = function(uglyArray) {
    // [0] -> 'object', [1] -> array of pairs
    var str = [];
    var pairs = uglyArray[1];
    var maxProps = Math.min(pairs.length, Nonymous.objPropNameClip);
     for (var i = 0; i < maxProps; i++) {
      var pair = pairs[i];
      str.push(pair[0].toString().substr(0, Nonymous.objPropNameClip));
    }
    return '{' + str.join(',') + '}';
  };
  // ----------------------------------------------------------------------------------------------
  ...
  function getFullHierarchy(name, namingStack, parent) {
    ...
    if (parentIndex < namingStack.length) {  // then we have context to consider
      ancestry = getHierarchyName(namingStack.slice(parentIndex));
    } else {
      ancestry = [];
    }
    ancestry.push(name);
  ...
  function convertToName(infos, namingStack) {
    ...
    return {localName: name, parent: parent, namingStack: namingStack};
  }
  ...
  Nonymous.assignmentLikeTypes = {
    'assign': function(functionInfo, parents) {
      functionInfo.isAssignment = true; 
      // 'assign': ['um', LITERAL, 'left', NODE, 'right', NODE],
      // build name from 'left' node, see table 4, case 3
      var nameBranch = functionInfo.parent[2]; 
      functionInfo.id = generateName(nameBranch);
    }, 
    'call': function(functionInfo, parents) {
     // 'call': ['left', NODE, 'args', ARRAY],
      // build name from 'left' node, see table 4, case 2
      var nameBranch = functionInfo.parent[1]; 
      functionInfo.id = generateName(nameBranch);
    },
    'decl': function(functionInfo, parents) {
      functionInfo.isAssignment = true;
      // 'decl': ['left', LITERAL, 'right', NODE],
      // 'left node is literal, just use it as name
      functionInfo.id = functionInfo.parent[1];
    }, 
    'pair': function(functionInfo, parents) {
      functionInfo.isAssignment = true;
      functionInfo.id = functionInfo.parent[1];  // prop name
      if (!functionInfo.id) {
        functionInfo.id = "''"; // jQuery Sizzle uses a blank prop name
      }
    }, 
    'return': function(functionInfo, parents) {
  ...
  Nonymous.getExpressionName = function(node, parents) {
    ...
    var functionInfo = new FunctionInfo(iter, node);
    var parentType = functionInfo.parentType;
    if (Nonymous.assignmentLikeTypes.hasOwnProperty(parentType)) {
      Nonymous.assignmentLikeTypes[parentType](functionInfo, parents);
      summary.push(functionInfo);
    }
