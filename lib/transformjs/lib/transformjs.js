 ...
 * - Convert to RequireJS module
 * - Export some constructor functions for use by outliner (TODO: avoid this)
 * - Add workaround for issue https://github.com/joehewitt/transformjs/issues/2
 * - Comment out logDeep since it causes requirejs to attempt to load util.js
 * - Comment out dandy
 */
...
function CATCH(val, from) {
    if (from) {
    	...
    	// XXX mamacdon: temp fix for https://github.com/joehewitt/transformjs/issues/2
        if (!val) {
            return null;
        } else {
...
// mamacdon TODO: rewrite outliner to avoid having to export these 
